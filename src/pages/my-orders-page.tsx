...
export function MyOrdersPage() {
  ...
  const cancelByInitiator = orderStates?.find((state: OrderState) => state.name === TASK_STATES.CANCELLED_BY_INITIATOR);
  const pendingConfirmation = orderStates?.find((state: OrderState) => state.name === TASK_STATES.PENDING_CONFIRMATION);
  const closed = orderStates?.find((state: OrderState) => state.name === TASK_STATES.CLOSED);
