...
export function SupportGeneralTab({ order, disabled, onUpdate }: SupportGeneralTabProps) {

  const [editedRequest, setEditedRequest] = useState<Order | null>(order);

  const [isServiceDialog, setServiceDialog] = useState(false);
  ...
  //const hasChanges = JSON.stringify(editedRequest) !== JSON.stringify(order);                      // флаг наличия изменений
  const hasChanges = useMemo(() => {
    if (!editedRequest || !order) return false;

    return (
      editedRequest.name !== order.name ||
      editedRequest.comment !== order.comment ||
      editedRequest.description !== order.description ||
      editedRequest.dateCreated !== order.dateCreated ||
      editedRequest.dateFinishPlan !== order.dateFinishPlan ||
      editedRequest.dateFinishFact !== order.dateFinishFact ||
      editedRequest.datePostpone !== order.datePostpone ||
      editedRequest.dateTechReturn !== order.dateTechReturn ||
      editedRequest.orderTypeId !== order.orderTypeId ||
      editedRequest.catalogItemId !== order.catalogItemId ||
      editedRequest.serviceId !== order.serviceId ||
      editedRequest.orderStateId !== order.orderStateId ||
      editedRequest.orderPriorityId !== order.orderPriorityId ||
      editedRequest.creatorId !== order.creatorId ||
      editedRequest.initiatorId !== order.initiatorId ||
      editedRequest.dispatcherId !== order.dispatcherId ||
      editedRequest.resultText !== order.resultText
    );
  }, [editedRequest, order]);
